...
function DisciplineDropdown({ placeholder, options, isSearchable, isMulti }) {
            ...
            return (
                <div className="filter_cont_discipline_multiSelection">
                    {selectedValue.map((option) => (
                        <div key={option.value} className="filter_cont_discipline_multiSelection_singleOption">
                            {option.label}
                            <span onClick={(event) => optionRemove(event, option)} className="filter_cont_discipline_multiSelection_singleOption_closeBtn"><CloseButton /></span>
                        </div>
                    ))}
                </div>
            );
    ...
    const isSelected = (option) => {
        if (isMulti) {
            return selectedValue.filter((opt) => opt.value === option.value).length > 0;
        }
    ...
    return (
        <div className="filter_cont_discipline_dropdown">
            <div ref={inputRef} onClick={handleInputClick} className="filter_cont_discipline_dropdown_selector">
